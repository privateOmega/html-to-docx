    ...
    this.numberingObjects.forEach(
      ...
      ({ numberingId, listElements }) => {
        ...
        listElements
          ...
          .forEach(({ level, type, properties }) => {
            const getListStyleType = (style) => {
            ...
            const getAttributes = (attr, key) => {
              if (!attr) return null;
              return attr[key] || null;
            };

            const dataStart = getAttributes(properties.attributes, 'data-start') || 1;
            ...
            const levelFragment = fragment({
              ...
              .att('@w', 'ilvl', level)
              .ele('@w', 'start')
              .att('@w', 'val', type === 'ol' ? dataStart : '1')
              .up()
              .ele('@w', 'numFmt')
              .att('@w', 'val', type === 'ol' ? getListStyleType(properties.style) : 'bullet')
              .up()
              .ele('@w', 'lvlText')
